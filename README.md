...
nmrsync is a bash script for periodic synchronization of Bruker NMR data to a data server 
...
options. The script searches for files that have been modified recently (< x days/hr/min
as specified in the input file), searching in folders up to the level of 
...
This can also be run (usually with the -full flag) occasionally to back up pp/wave/par 
folders from the Topspin directories if desired.
...
and RHEL7.3 on the remote side. I've only used this with Bruker NMR data, but future releases 
may be able to handle file structures generated by other instruments.
...
Options
 ...
 `-m, --manual`              		 Manual mode: enter password instead of using SSH 
                                         keys--not recommended.
										 
 `-p, --processed`         		 Processed data mode: will ignore excludelist.instrument 
                                         list in the input folder and copy processed data.
										 
 `-f, --full`                		 Full mode: copy over all data instead of just
                                         recently added data.
